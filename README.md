...
- 云同步（JSONBin）：
  ...
  - 一键“同步（合并）”：按计数器 id 合并，冲突以更新时间较新的为准；历史以时间戳为唯一键去重（同一时间戳两端不一致时以较新一侧覆盖）。
...
## 变更记录（节选）
- v1.3.0
  - 同步可靠性：新增“删除墓碑”（计数器与历史记录），合并时会尊重墓碑并删除云端对应项，不再“被拉回”。
  - 计数一致性：合并改为“先合并历史→按历史重算计数（仅统计 delta>0）”，删除历史/撤销后计数正确回退并跨设备一致。
  - 撤销一致性：本地“−1”会删除最近一条 +1 历史并写入墓碑，合并后其它设备也会同步删除。
  - 体验优化：添加“记录”（0 增量）后在“−1”前显示短暂提示；首次打开设备自动弹出“使用帮助”。
  - 视觉微调：云图标居中，旋转更对称。
  - PWA 缓存：提升 SW 缓存版本至 v1.3.0（安装版用户重开一次应用即可获取最新资源）。
- v1.1.0
...
- 合并策略：
  ...
  - 历史以 `ts` 为唯一键去重并按时间倒序；当同一 `ts` 两端的 `delta/note` 不一致时，采用 `updatedAt` 较新的一侧（基准侧）覆盖；支持“删除墓碑”，可跨设备删除历史项。
...
## 变更记录（节选）
- v1.3.3（2025-09-10）
  - 重构（帮助页）：改为“卡片式分区”，新增并按顺序展示：这是什么 → 常见场景 → 基础功能 → 云同步（可折叠） → 应用安装（PWA，可折叠） → 隐私与数据（可折叠）；标题改为“欢迎使用 · 计数伴侣”。
  - 文案（云同步）：恢复保姆级步骤说明，补充“测试连接 404 常因 Bin 未创建”；按钮说明/多设备同步等信息更清晰。
  - 复制（默认 JSON）：在同步说明的默认 JSON 代码框内置“复制”按钮（无悬浮提示，点击后短暂显示“已复制”）。
  - 安装（PWA）：新增“应用安装”折叠区，说明 Chrome/Edge 支持、电脑与手机的安装入口；补充 Google Play 环境下以 WebAPK 集成的说明。
  - 隐私：合并到帮助页的“隐私与数据”折叠；移除页脚“隐私”入口与独立隐私对话框；补充“解除绑定仅清除本机密钥、不删除云端数据”“离线可用、恢复后自动重试”等说明。
  - 交互：帮助内折叠内容加入淡入/上滑开合动画；“复制默认 JSON”点击反馈更明确，无多余弹窗。
  - 排版：帮助页段落与列表行距优化；分区卡片添加浅背景/边框与左侧强调线，代码框右上角放置复制按钮。
  - 文案细节：基础功能将“创建目标”改为“创建新计数器”；“常见场景”上移到“这是什么”之后。
- v1.3.2（2025-09-01）
  - 修复（归档切换）：切换到归档视图后卡片初始短暂显示亮色的问题，现已在动画保留的前提下，首帧即呈现灰度外观。
  - 修复（云同步记录）：编辑备注或删除历史后导致同一条记录在同步后分裂为两条的问题。历史以时间戳 ts 为唯一键去重，冲突由更新时间较新的一侧覆盖；计数仍按合并后的历史重算。
- v1.3.1（2025-08-30）
  - 修复（三点菜单·移动端）：展开时无阴影、关闭时阴影延迟消退。调整触屏下的 hover 屏蔽规则，排除 `[aria-expanded="true"]` 与点击反馈态（`.tap-flash`）；关闭菜单时立即移除点击高亮。
  - 修复（按钮反馈·移动端）：“新建/查看归档/主题/云/−1/+1”等点击类按钮仅第一次有阴影、之后没有的问题。统一触屏 hover 屏蔽与点击反馈的优先级，确保每次点击都有一致的短反馈。
  - 统一（点击反馈时长）：新增 `TAP_FLASH_MS = 180ms`，所有点击反馈动画采用固定时长，独立于具体 CSS 过渡；三点菜单关闭立即取消高亮以对齐桌面“移开即取消”的感觉。
  - 修复（+1 重复）：在支持 Pointer Events 的设备上不再同时绑定 touch 回退，避免一次点击加两次；仅在不支持 Pointer Events 的旧设备启用 touch 回退。
  - 兼容（黑框问题）：部分浏览器对 `color-mix/oklab` 支持不完整导致卡片出现黑色内框；改为 `@supports` 有才启用内描边，否则退回无内描边；统一输入框/开关/聚焦描边的回退为主题色。
  - 视觉（粉色主题徽章）：修复右侧边缘“没画完”的像素缝；改用内阴影模拟 1px 边框并启用 `background-clip: padding-box`。
- v1.3.0
  - 新增“归档”功能：在卡片“更多”中可归档/取消归档；工具栏新增“查看归档”切换视图；归档计数器不可 +1/−1/记录/历史编辑，需取消归档后操作。
  - 交互动效：归档/取消归档采用平滑离场动画；切换“查看归档/返回”在两端（包括空列表）都有进入动画；移动端在“降低动态效果”偏好下保留卡片的关键动效。
  - 云同步：`archived` 状态已纳入合并与写入；修复“取消归档后立即同步可能不同步”的时序问题（先标记保存再做离场动画）。
  - 统一弹窗：归档提示改为应用内 `infoDialog`，不再使用浏览器原生 alert。
- v1.2.0
